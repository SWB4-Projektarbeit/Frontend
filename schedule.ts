...
const fallbackData: SchedulePayload = {
...
function createEntryNode(entry: ScheduleEntry) {
  if (!(scheduleTemplate instanceof HTMLTemplateElement)) {
    return document.createElement("div");
  }

  const clone = scheduleTemplate.content.cloneNode(true) as DocumentFragment;
  ...
  if (badgeEl) {
    if (entry.status) {
      ...
      (badgeEl as HTMLElement).style.display = "inline-flex";
    } else {
      badgeEl.remove();
    }
  }
...
function renderSchedule(data: SchedulePayload) {
...
async function fetchSchedule(url: string): Promise<SchedulePayload> {
